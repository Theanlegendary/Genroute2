...
const spatialIndexer = require('./spatial_branch_indexer');
...
/**
 * Must call init() once after server loads flatNcddList and pickupBranches.
 */
function init({ flatNcddList, pickupBranches, stripAdministrativePrefixes }) {
  _flatNcddList = flatNcddList || [];
  _pickupBranches = pickupBranches || [];
  _stripAdministrativePrefixes = stripAdministrativePrefixes || ((s) => s);
}

// ── Improvement 5 & Spatial Branch Integration: Enricher ─────────────────────────
function enrichWithNcddCodes(market) {
  if (!market) return market;
  let enriched = { ...market };

  // 1. Spatial Branch 10km Auto-Selection
  if (_pickupBranches.length > 0 && enriched.latitude && enriched.longitude) {
    enriched = spatialIndexer.enrichLocationWith10kmBranch(enriched, _pickupBranches, 10.0);
  }

  // 2. NCDD Administrative Codes Enrichment
